 ...
 * @LastEditTime: 2021-02-04 14:49:47
-->
<template>
  <div>
    <a-list v-if="showType==0" itemLayout="horizontal" :dataSource="data">
    ...
    <ResetPassword v-if="showType==1" @return_btn="showType=0" />
  </div>
</template>

<script>
import ResetPassword from "./SecurityPages/resetPassword";
...
export default {
  ...
  created() {
    this.principal = store.getters["account/user"];
    let passwordRule = store.getters["account/user"].passwordRule;
    this.data[0].value =
      passwordRule == 1 ? "弱" : passwordRule == 2 ? "中" : "强";
  },
  methods: {},
};
</script>

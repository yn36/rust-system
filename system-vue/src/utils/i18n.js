...
function generateI18n(lang, routes, valueKey) {
  routes.forEach(route => {
    ...
    if (route.meta && route.meta.i18n) {
      value = route.meta.i18n[valueKey] || route['name']
    }
...
function mergeI18nFromRoutes(i18n, routes) {
  formatFullPath(routes)
  const CN = generateI18n(new Object(), routes, 'CN')
  const US = generateI18n(new Object(), routes, 'US')
  const HK = generateI18n(new Object(), routes, 'HK')
  i18n.mergeLocaleMessage('CN', CN)
  i18n.mergeLocaleMessage('US', US)
